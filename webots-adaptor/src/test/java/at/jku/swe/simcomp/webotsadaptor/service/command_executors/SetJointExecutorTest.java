...
public class SetJointExecutorTest {
    ...
    void executeCommandTest() throws RoboOperationFailedException, IOException, ParseException {
        try (MockedStatic<WebotsExecutionService> mockedService = Mockito.mockStatic(WebotsExecutionService.class)) {
            ...
            ExecutionCommand.SetJointPositionCommand command = new ExecutionCommand.SetJointPositionCommand(
                    new JointPositionDTO(RoboJoint.AXIS_1, 1.0));

            SetJointExecutor executor = new SetJointExecutor();
